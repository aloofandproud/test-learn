...
import com.immoc.diners.config.OAuth2ClientConfiguration;
import com.immoc.diners.domain.OAuthDinerInfo;
import com.immoc.diners.mapper.DinersMapper;
import com.immoc.diners.vo.LoginDinerInfo;
...
//    @Value("${service.name.ms-oauth-server}")
    private String oauthServerName = "http://ms-oauth2-server/";
    ...
    //验证
    public void checkPhoneIsRegisterd(String phone){
        AssertUtil.isNotEmpty(phone,"手机号不能为空");
        Diners diners = dinersMapper.selectByPhone(phone);
        AssertUtil.isTrue(diners==null,"该手机号未注册");
        AssertUtil.isTrue(diners.getIsValid()==0,"该用户已锁定");
    }
    @Resource
